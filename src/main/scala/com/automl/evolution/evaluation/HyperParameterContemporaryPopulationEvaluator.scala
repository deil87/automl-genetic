    ...
    tPopulation.individuals.foreach(template => template.setLogPadding(currentLogPadding))
      ...
      population.individuals.map { hpField =>
        ...
        val fitness = cache.getOrElseUpdate(cacheKey, {
          ...
          val threeBestEvaluations = tPopulation.individuals.map(template => template.evaluateFitness(trainingSplit, testSplit, problemType, hyperParamsMap = hpField).getCorrespondingMetric)
