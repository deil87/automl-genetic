...
class MisclassificationDistance extends PhenotypicDistance[DataFrame, MisclassificationDistanceResult] with LazyLogging {
  ...
  def getDistance(df1: DataFrame, df2: DataFrame): MisclassificationDistanceResult = {
    ...
    val cosineDistanceBasedOnErrors = {
      ...
      if (cosineDistance < 1e-5) 0 else cosineDistance // TODO do we need this?
    }
    ...
    MisclassificationDistanceResult(cosineDistanceBasedOnErrors, 0, 0, withCosineDistance)
  }
